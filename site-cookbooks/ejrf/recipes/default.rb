...
%w{ build-essential python-dev python-pip python-setuptools python-pip libpq-dev libxml2 libxml2-dev libxslt1-dev }.each do |pkg|
    package pkg do
	action :install
    end
end
...
directory "/home/#{node["user"]["name"]}/app/venv" do
    owner node["user"]["name"]
    mode 00644
    action :create
    recursive true
